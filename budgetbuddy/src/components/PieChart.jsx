      ...
      {chartData && (
        <Card className="flex flex-col w-full md:w-[32vw] px-2 rounded-2xl">
          ...
          <CardFooter className="hidden md:flex md:flex-col gap-2 text-sm  items-start h-[50%] py-2">
            {colorDescriptions && colorDescriptions.map((el, i) => {
              return (
                <div key={i} className={`flex flex-col items-start gap-5`}>
                  <div className="flex gap-1 items-center">
                  <p style={{backgroundColor: el.color}} className={`w-3 h-[50%]  rounded-sm`}></p>
                  {/* {console.log(JSON.stringify(el.color), "INI WARNAAA")} */}
                    <p>Total {el.label} </p>
                    {/* <p>{el.color}</p> */}
                  </div>
                </div>
